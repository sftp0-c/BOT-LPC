...
from handlers.common import BACK, DEFAULT_WELCOME, admin_of, is_super, need_super, notify
...
async def cb_staff_stats(x, arg):
    ...
    await common.api.send(x, f"📊 Ваши обращения\n{counts}", BACK)
...
async def cb_admins(x, arg):
    ...
    await common.api.send(x, text, [*kb, [btn("➕ Добавить сотрудника", "sfadd")], *BACK])


async def send_staff_card(x: str, staff_id: str):
    a = await admin_of(staff_id)
    if not a or is_super(a):
        return await common.api.send(x, "Сотрудник не найден.", [[btn("↩️ К списку", "admins")]])
    text = (
...
async def cb_staff_delete_ask(x, arg):
    a = await admin_of(arg)
    if await need_super(x) and a and not is_super(a):
        await common.api.send(x, f"Удалить сотрудника {a['full_name']}?",
...
async def cb_staff_delete(x, arg):
    ...
    await common.api.send(x, "✅ Сотрудник удалён.", [[btn("↩️ К списку", "admins")]])
...
async def cb_staff_add(x, arg):
    ...
    await common.api.send(x, "Введите MAX ID сотрудника (цифры). Сотрудник может узнать свой ID, написав боту /id. (или /cancel)")
...
async def st_add_staff_id(x, text, p):
    ...
    if not staff_id.isdigit():
        return await common.api.send(x, "ID состоит только из цифр. Попробуйте ещё раз.")
    if await admin_of(staff_id):
        await db.clear_state(x)
        return await common.api.send(x, "Этот пользователь уже в списке сотрудников/сис-админов.", [[btn("↩️ К списку", "admins")]])
    await db.set_state(x, "add_staff_name", {"id": staff_id})
    await common.api.send(x, "Введите ФИО сотрудника (как его увидят студенты).")
...
async def st_add_staff_name(x, text, p):
    ...
    name = short(text, 100)
    if not name:
        return await common.api.send(x, "Введите ФИО сотрудника.")
    ...
    await common.api.send(x, "✅ Сотрудник добавлен." + ("" if delivered else "\n⚠️ Он пока не запускал бота — пусть нажмёт «Начать»."))
...
async def cb_schedules(x, arg):
    ...
    await common.api.send(x, text, [*kb, [btn("➕ Добавить / изменить", "scadd")], *BACK])
...
async def cb_schedule_add(x, arg):
    if await need_super(x):
        ...
        await common.api.send(x, "Введите код группы, например ИС-21 (или /cancel).")
...
async def cb_schedule_edit(x, group):
    if await need_super(x):
        ...
        await common.api.send(x, f"Отправьте новую ссылку на расписание группы {group} (http/https).")
...
async def st_sc_group(x, text, p):
    ...
    if not 1 <= len(group) <= 30:
        return await common.api.send(x, "Код группы — до 30 символов.")
    await db.set_state(x, "sc_url", {"group": group})
    await common.api.send(x, f"Отправьте ссылку на расписание группы {group} (http/https).")
...
async def st_sc_url(x, text, p):
    ...
    if not is_http_url(text):
        return await common.api.send(x, "Нужна ссылка вида https://… Попробуйте ещё раз.")
    await repo.upsert_schedule(p["group"], text.strip())
    await db.clear_state(x)
    await common.api.send(x, f"✅ Расписание группы {p['group']} сохранено.")
    await cb_schedules(x, "")
...
async def cb_set(x, arg):
    ...
    if arg == "tickets":
        ...
        await common.api.send(x, "Отправьте новый текст приветствия студентов (до 500 символов) или /cancel.")
