...
from handlers.common import BACK, admin_of, can_broadcast, notify, spawn
...
async def cb_broadcast(x, arg):
    if not can_broadcast(await admin_of(x)):
        return await common.api.send(x, "У вас нет права на рассылку.", BACK)
    await common.api.send(x, "📢 Кому отправить?",
...
async def cb_broadcast_audience(x, arg):
    ...
    if arg == "group":
        ...
        return await common.api.send(x, "Введите код группы (или /cancel).")
    await db.set_state(x, "bc_text", {"aud": "all"})
    await common.api.send(x, "Введите текст рассылки для всех студентов (или /cancel).")
...
async def st_bc_group(x, text, p):
    ...
    if not n:
        return await common.api.send(x, f"В группе «{group}» нет зарегистрированных студентов. Проверьте код и введите ещё раз.")
    await db.set_state(x, "bc_text", {"aud": group})
    await common.api.send(x, f"Группа {group}: получателей {n}. Введите текст рассылки (или /cancel).")
...
async def st_bc_text(x, text, p):
    ...
    await common.api.send(x, f"Отправить {where} ({n} чел.)?\n\n📢 Объявление колледжа:\n\n{text}",
                   [[btn("✅ Отправить", "bcgo"), btn("❌ Отмена", "home")]])


@state("bc_confirm")
async def st_bc_confirm(x, text, p):
    await common.api.send(x, "Нажмите «✅ Отправить» или «❌ Отмена» под сообщением с текстом рассылки (либо /cancel).")
...
async def cb_broadcast_go(x, arg):
    ...
    if not (can_broadcast(await admin_of(x)) and st and st["state"] == "bc_confirm"):
        return await common.api.send(x, "Нет подготовленной рассылки. Начните заново.", BACK)
    await db.clear_state(x)
    spawn(run_broadcast(x, st["payload"]["aud"], st["payload"]["text"]))
    await common.api.send(x, "⏳ Рассылка запущена, пришлю итог по завершении.")
